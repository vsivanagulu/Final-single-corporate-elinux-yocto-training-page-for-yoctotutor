...
const SectionTitle: React.FC<{ title: string; subtitle: string }> = ({ title, subtitle }) => (
    <div className="text-center mb-12">
      ...
      <p className="mt-6 text-lg text-white/90">{subtitle}</p>
    </div>
);
...
const Contact: React.FC = () => {
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        // Reset form or send data to a server here
    ...
    return (
        <section id="contact" className="py-20 bg-gradient-primary text-white">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <SectionTitle title="Connect With Us" subtitle="Let's discuss bringing up your custom hardware with Yocto and Linux" />
                ...
                <div className="grid lg:grid-cols-2 gap-12 max-w-5xl mx-auto items-start">
                    <div className="space-y-8">
                            ...
                            <div className="space-y-6">
                                {contactInfo.map((item, index) => (
                                    <div key={index} className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <h4 className="text-sm opacity-90">{item.label}</h4>
                                            {item.href ? (
                                                <a href={item.href} className="text-lg font-medium hover:underline">{item.value}</a>
                                            ) : (
                        ...
                        <div className="bg-white/10 backdrop-blur-sm p-8 rounded-xl">
                            <h3 className="font-display text-2xl font-semibold mb-6">Connect on WhatsApp</h3>
                            <div className="flex flex-col sm:flex-row items-center gap-6 text-center sm:text-left">
                                <img 
                                    src="https://i.ibb.co/68gCj3J/whatsapp-qr.png" 
                                    alt="WhatsApp QR Code for Embedded Linux Training" 
                                    className="w-32 h-32 rounded-lg object-cover flex-shrink-0 border-2 border-white/20" 
                                />
                                <div>
                                    ...
                                    <a 
                                        ...
                                        className="inline-flex items-center gap-2 bg-green-500 text-white font-semibold py-2 px-5 rounded-full shadow-lg transition-all duration-300 hover:scale-105 hover:bg-green-600"
                                    >
                                        <WhatsAppIcon className="w-6 h-6" /> Chat Now
                                    </a>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="bg-white p-8 rounded-xl shadow-2xl">
                        <form onSubmit={handleSubmit} className="space-y-6">
                            <div>
                                <label className="block text-sm font-medium text-dark-text mb-2">Full Name <span className="text-accent">*</span></label>
                                <input type="text" placeholder="Your full name" required className="w-full px-4 py-3 border-2 border-gray-200 rounded-lg text-dark-text focus:outline-none focus:ring-2 focus:ring-secondary focus:border-transparent transition" />
                            </div>
                             <div>
                                <label className="block text-sm font-medium text-dark-text mb-2">Email <span className="text-accent">*</span></label>
                                <input type="email" placeholder="Your email" required className="w-full px-4 py-3 border-2 border-gray-200 rounded-lg text-dark-text focus:outline-none focus:ring-2 focus:ring-secondary focus:border-transparent transition" />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-dark-text mb-2">Phone <span className="text-accent">*</span></label>
                                <input type="tel" placeholder="Your phone" required className="w-full px-4 py-3 border-2 border-gray-200 rounded-lg text-dark-text focus:outline-none focus:ring-2 focus:ring-secondary focus:border-transparent transition" />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-dark-text mb-2">Organization</label>
                                <input type="text" placeholder="Company or college" className="w-full px-4 py-3 border-2 border-gray-200 rounded-lg text-dark-text focus:outline-none focus:ring-2 focus:ring-secondary focus:border-transparent transition" />
                            </div>
                            <button type="submit" className="w-full flex items-center justify-center gap-2 bg-gradient-accent text-white font-semibold py-3 px-6 rounded-lg text-lg shadow-lg transition-all duration-300 hover:scale-105 hover:shadow-xl">
                               <PaperAirplaneIcon className="w-5 h-5" /> Request a Quote
                            </button>
                        </form>
                        {formSubmitted && (
                            <div className="mt-4 flex items-center gap-3 bg-secondary text-white p-4 rounded-lg">
                                <CheckCircleIcon className="w-6 h-6" />
                                <span>Thank you! We will contact you shortly with a quote.</span>
                            </div>
                        )}
                    </div>
                </div>
